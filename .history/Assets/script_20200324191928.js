...
function openForm() {
    ...
    _formClickListener = closeButton.addEventListener("click", function () {
        closeForm();
        event.target.removeEventListener("click", _formClickListener);
    })
    _formKeydownListener = closeButton.addEventListener("keydown", formKeydownHandler)
      
  //  Set popup visibility
  function closeForm() {
    let initialTag = document.getElementById("initials");
