...
function initPictureList() {
    let returnArray = pictureList();

    for (let i = 0; i < returnArray.length; i++) {
        if (returnArray[i].filepath == "") {
            returnArray[i].filepath = _pictureDefaultPath;
        };
    };
...
function renderQuestion (targetIndex) {
    ...
    let newPicture = getPictureByID(questionObject[_pictureID], _pictureArray);
    let pictureFilepath = "";
    let pictureTitle = "";
    let captionText = "";
